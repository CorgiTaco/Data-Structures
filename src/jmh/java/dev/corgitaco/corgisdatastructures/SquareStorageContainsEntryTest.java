...
public class SquareStorageContainsEntryTest {
    ...
    public void long2ObjMapContains() {
        ...
        for (int x = 0; x < side; x++) {
            for (int y = 0; y < side; y++) {
                long key = pack(x, y);
                boolean b = L2I_STORAGE.containsKey(key);
                if (b) {
                    count++;
                }
            }
    ...
    public void nearestPointContains() {
        ...
        for (int x = 0; x < side; x++) {
            for (int y = 0; y < side; y++) {
                SimplePosition2D position = new SimplePosition2D(x, y);
                Target<SimplePosition2D, SimplePosition2D> target = QUAD_TREE_NEAREST_POSITION.getTarget(position);
                if (target != null) {
                    count++;
                }
            }
